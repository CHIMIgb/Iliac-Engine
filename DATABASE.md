...
| **Criterio de aceptación** | ✅ Publicar → aparece públicamente con slug único (`SLUG_TAKEN` en conflicto; autogenerado con sufijo `-2/-3` si colisiona); despúblicar → desaparece de la galería (y `GET /:slug` → 404); crear proyecto desde `tpl-demo` carga la sala jugable (validada por el contrato y probada por API). Tests: **47/47** (11 nuevos en `gallery-templates.test.ts`: publish, slug en conflicto, visitas 1→2, unpublish idempotente, ajeno → 404, plantilla válida, POST con plantillaId, 404 plantilla inexistente, público sin token). |
