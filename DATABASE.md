...
>
> **Actualizado 2026-09-18 (ASSET_UPLOAD_PLAN.md):** estructura de blobs ordenada por usuario y tipo — `storage/uploads/<userId>/<tipo>/<assetId>.<ext>` — y **dedupe por hash por usuario** (el mismo contenido en otra cuenta crea fila y blob propios; aislamiento multi-cuenta). Migración one-shot: `npm run migrate:uploads` (mueve blobs existentes y actualiza `asset.ruta`).
>
...
>
> **Fix 2026-09-17 (validación de C5e) — «al iniciar sesión dejan de funcionar todas las herramientas»:** el reload caro del viewport creaba un segundo `WebGLRenderer` sobre el mismo canvas (el viejo seguía vivo) y el frame roto se llevaba por delante `tm.update()`/`overlay.draw()`; además `applyDoc` pisaba los suscriptores del documento (`Object.assign`) y el estado vacío no traía `render`. Arreglado con `renderSignature()` (firma estable), orden `dispose → createEngine → load`, `EditorState.applyFrom()` y `fromProjectJson` respetando el default. Studio **254/254**, motor **192/192** (ver sub-bloque C5e). **Fix C4 (mismo día):** crear desde plantilla guardaba el `meta.name` de la plantilla en vez del nombre del proyecto (columna y documento desalineados, toolbar con el nombre viejo); ahora el clon nace con `data.meta.name = nombre` y hay test de regresión (server **68/68**).
>
...
**Validación de refresh:** existe (`token_hash`), no revocado (`revocado_en IS NULL`), no expirado (`expira_en > now()`). **Rotación:** al refrescar se revoca el viejo (`revocado_en = now()`) y se crea uno nuevo. **Logout (C5g):** **borra** la fila del refresh de esa sesión (no la marca: una fila revocada que llega a `/auth/refresh` es la señal de ROBO que tumba todas las sesiones, y un cliente legítimo puede reintentar con el refresh de una sesión que acaba de cerrar en otra pestaña) + inserta el `jti` del access presentado en `token_invalido` con su `exp`. **Purga:** perezosa — al insertar en `token_invalido` se borran las filas con `expira_en < now()` (sin cron por ahora).
**Denylist en uso (C5g):** `lib/auth.ts` → `verifyAccessToken()` (firma + `exp` + lookup por `jti` en `token_invalido`) es el ÚNICO punto de validación de access tokens; `requireAuth`/`optionalAuth` y todo endpoint futuro pasan por él (regla de `AGENTS.md`). Coste: 1 consulta por petición autenticada (lookup por `jti` UNIQUE); solo se escribe al cerrar sesión.
**Límite conocido:** un access **nunca visto** por el servidor no se puede denylistar (no hay relación `jti`↔sesión). Cuando el anti-robo revoca todas las sesiones, los access ya emitidos siguen sirviendo hasta su `exp` (≤15 min) — lo que muere al instante es la capacidad de renovar.
...
| | |
|---|---|
| **Qué se creó** | Subida/lectura/borrado de assets sobre la tabla `asset` (`src/routes/assets.ts` + `src/lib/storage.ts` + `src/schemas/assets.ts`): `POST /api/assets` (multipart: `file`+`tipo`+`proyectoId?`), `GET /api/assets/:id` (metadata), `GET /api/assets/:id/file` (bytes con `Content-Type` real), `DELETE /api/assets/:id` (fila + archivo). El **MIME se detecta por magic bytes** (`file-type`, nueva dep) — nunca se confía en la extensión ni en el header del navegador — y el `tipo` declarado debe encajar con él (422 si no). **Tope 20 MB** → `413 ASSET_TOO_LARGE` (nuevo código). **Dedupe por hash sha256 POR USUARIO (ASSET_UPLOAD_PLAN.md, 2026-09-18)**: re-subir el mismo archivo **en la misma cuenta** devuelve el asset existente (200) sin duplicar bytes ni fila; el mismo contenido en otra cuenta crea fila y blob propios (aislamiento multi-cuenta). **Blob en `storage/uploads/<userId>/<tipo>/<id>.<ext>`** — ruta resuelta con `import.meta.dirname`, configurable por `STORAGE_PATH` (los tests usan un `mkdtemp` limpio); migración one-shot de la estructura plana anterior: `npm run migrate:uploads` (`server/src/scripts/migrate-uploads.ts`, idempotente). |
| **Criterio de aceptación** | ✅ Subir el PNG del Sprite Tool (`guard_f0`) → fila `asset` (`tipo: sprite`, `mime: image/png`, hash sha256) + blob servido por `/file` con **bytes idénticos** (round-trip verificado por curl y por test); re-subir el mismo archivo → `reused: true` con el mismo id; dos cuentas subiendo el mismo archivo → ids distintos y el borrado de una no rompe el asset de la otra (test de aislamiento); borrar elimina fila + archivo (GET posterior → 404 `ASSET_NOT_FOUND`). Tests: **77/77** (test de aislamiento añadido en ASSET_UPLOAD_PLAN.md paso 5). |

### C4 — Galería y plantillas: `galeria`, `plantilla` — ✅ realizada

| | |
|---|---|
| **Qué se creó** | Publicación/despublicación transaccional en `routes/projects.ts`: `PATCH /api/projects/:id/publish` (upsert en `galeria` + `estado=PUBLICADO` + `publishedAt`; slug autogenerado desde `nombre` o explícito; conflicto → `SLUG_TAKEN`; **revalida `data` con `validateProject`** antes de publicar) y `PATCH /api/projects/:id/unpublish` (revertir, **idempotente**). Galería pública sin auth en `routes/gallery.ts`: `GET /api/gallery` (lista con autor, sin `data`) y `GET /api/gallery/:slug` (data completo para el motor + **incrementa `visitas`**). Plantillas públicas en `routes/templates.ts`: `GET /api/templates` (sin data) y `GET /api/templates/:id` (con data; inexistente → `404 TEMPLATE_NOT_FOUND`, código nuevo). `POST /api/projects` acepta `plantillaId` → crea con **copia** de `plantilla.data` (nombre hereda). El seed `tpl-demo` pasó de ser un cascarón a una **sala v3 jugable** (sector 10×10, 4 paredes, texturas color, `sky.set/frame`) que valida contra el contrato. **Fix 2026-09-17 (detectado al validar C5e):** el clon se guardaba con el `meta.name` de la plantilla, así que todo proyecto nacido desde `tpl-studio` quedaba con la columna `nombre` y el documento desalineados (`Escenario Studio` / `Proyecto nuevo`) y la toolbar del Studio pintaba el nombre viejo. Ahora `data.meta.name = nombre` (el del body o el de la plantilla) manteniendo el resto del árbol intacto; también se arregló el dato sucio de `9c908dce` por API. |
| **Criterio de aceptación** | ✅ Publicar → aparece públicamente con slug único (`SLUG_TAKEN` en conflicto; autogenerado con sufijo `-2/-3` si colisiona); despúblicar → desaparece de la galería (y `GET /:slug` → 404); crear proyecto desde `tpl-demo` carga la sala jugable (validada por el contrato y probada por API). Tests: **16** en `gallery-templates.test.ts` (15 previos + 1 de regresión `Fix C4`: sin nombre → `meta.name` de la plantilla y con nombre explícito → ese; el par falla si se revierte el fix), **68/68** del server. |
...
- **C5g ✅ realizada (2026-09-17) — logout real, denylist de access en uso y guard universal:** la tabla `token_invalido` estaba creada desde A1 y **nunca se escribía**: `requireAuth` validaba solo firma+`exp`, no existía `POST /auth/logout` y el botón Cuenta del Studio se limitaba a borrar la cookie (el refresh seguía vivo 7 días en `refresh_token`).
  - **Server (hecho):** `lib/auth.ts` gana **`verifyAccessToken(token)`** = firma + `exp` + lookup del `jti` en `token_invalido`; es el **único** punto de validación de access tokens (`requireAuth`/`optionalAuth` pasan por él y dejan `accessJti`/`accessExp` en el contexto). Nuevo **`POST /auth/logout`** (con `requireAuth`, `logoutSchema { refreshToken? }`): (1) **borra** la fila de `refresh_token` de esa sesión filtrando por hash **+ `usuarioId`** → cerrar sesión no cierra las demás y pasar el refresh de otro no revoca nada ajeno; (2) **upsert** del `jti` del access en `token_invalido` con su `exp` real → el access muere al instante (antes seguía sirviendo hasta 15 min); purga perezosa (`DELETE WHERE expira_en < now()`) en la misma transacción. `jwt.ts`: `TokenPayload` tipa `jti`/`exp` (ya se firmaban). Sin cambios de schema ni de contrato.
  - **Decisión clave (bug detectado en la verificación e2e):** el logout **borra** la fila en vez de marcarla `revocado_en`. Motivo: en `/auth/refresh`, una fila revocada = señal de robo → revoca TODAS las sesiones; con el logout marcando la fila, cualquier reintento legítimo con el refresh recién cerrado (otra pestaña, petición en vuelo) tumbaba al usuario en todos sus dispositivos. Con la fila borrada, ese refresh da `401` simple. La revocación global queda **exclusiva** del reuso de un refresh **rotado**. Cubierto por un test específico.
  - **En lo que NO se escribe (decisión del usuario, 2026-09-17):** los access/refresh **vencidos** no se guardan — un `exp` pasado ya lo rechaza `verifyToken`, y guardarlos sería una escritura en DB por cada token que expira sin aportar seguridad; los refresh vencidos ya viven en `refresh_token` con `expira_en`.
  - **Studio (hecho):** `io/api.ts` **`apiLogout(refreshToken)`** (+ `/auth/logout` en `NO_RENEW`: un 401 ahí no dispara renovación, evita el bucle); `main.ts` el botón Cuenta llama al servidor **antes** de `clearSession()` con `try/catch` (si la red falla se cierra en local igual: el usuario nunca queda atrapado).
  - **Tests:** server **76/76** (`tests/auth-logout.test.ts`, 8 nuevos: access muerto + fila en `token_invalido` con `expira_en = exp`; otras sesiones del MISMO usuario vivas; refresh ajeno no se revoca (filtro por dueño); sin body; sin token → 401 y repetido → 401; **refresco de una sesión cerrada no cierra las demás**; reuso rotado → global; **cobertura del guard**: recorre `app.routes` y exige 401 en todas las `/api/*` sin token salvo la lista blanca pública) + Studio **257/257** (`tests/api-logout.test.ts`, 3: cabecera/body correctos, 401 sin renovar, fallo de red → `NETWORK_ERROR` y cookie fuera) + `typecheck` y `vite build` OK.
  - **Verificación end-to-end (`curl.exe`):** dos sesiones de `chimi` → logout de la primera → su access y su refresh dan `401`, la segunda renueva con normalidad; `token_invalido` con 2 filas (`jti` + `expira_en` a ~15 min, usuario `chimi`) y `refresh_token` con vivos/rotados coherentes.
  - **Criterio de aceptación (cumplido):** cerrar sesión mata el access al instante, no toca las otras sesiones, y la revocación global solo salta con reuso de un refresh rotado.
  - **Análisis pedido (engine ↔ studio): no implementar** — no hay canal que asegurar: el Studio importa el motor como módulo del mismo bundle (`@engine/index.js`), mismo origen, sin red. Los canales reales son Studio↔API (JWT, cubierto) y el motor descargando blobs de `GET /api/assets/:id/file`, que es **público** (D1): quien tenga el UUID de un asset de un proyecto privado lo descarga (enlace-capacidad). Deuda anotada en §"Hueco futuro": assets privados / URLs firmadas; y si el Publisher llega a correr `project.json` de terceros, iframe con `sandbox` + `postMessage` con `origin` explícito.

### Hueco futuro (fuera de estas fases)

Roles `admin` vs `creador` aplicados por endpoint, thumbnails/galería con imágenes reales, rate limits globales, tests e2e Studio↔API, despliegue (Docker Compose completo). **Seguridad (anotado en C5g):** autorización por recurso en `GET /api/assets/:id/file` (hoy público: URL-capacidad; exigir sesión/propiedad o firmar la URL para assets de proyectos no publicados) y, si el Publisher ejecuta `project.json` de terceros, iframe con `sandbox` + `postMessage` con `origin` explícito. Nada de esto bloquea las fases A–C.
