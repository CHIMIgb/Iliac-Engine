...
| F4.7 **Ciclo día/noche e iluminación dinámica — cielo realista**: `world.sky.style:'realista' {hour, dayLengthSec?, shadows?, sunTilt?}` (aditivo; sin style = clásico intacto). **`engine/core/daylight.js`** (puro, sin Three): `sunDirection/moonDirection` (anti-solar), `sunElevation`, `paletteFor` (curvas sol/ambiente/niebla), `advanceHour` (día solar en segundos), `hourLabel`; **`engine/three/SunSystem.js`**: shader atmosférico oficial `three/addons/objects/Sky.js` (Rayleigh → atardeceres), discos de sol/luna (sprites opacos con alphaTest, `depthTest:false` + `renderOrder` negativo → pintados detrás del mundo, nada se cuela en interiores), estrellas (Points que se encienden de noche por tamaño), `DirectionalLight` con `shadowMap` PCF 2048 (caja ortográfica 60 m siguiendo al jugador) + `HemisphereLight`; **`Engine3D._setupSun()`** (firma con `sunSignature`, enclava/desenclava `renderer.setDefaultLights`), avance del reloj en `update()` (solo playtest), interior automático por `ceilTex !== 'sky'` (interiores atenúan el sol ×0.25 — Daggerfall-style); `Renderer3D` activa sombras dinámicas y tiñe la niebla con `paletteFor().fogColor`; `WorldMesh`/`StairsMesh` con `castShadow/receiveShadow` (2 líneas aditivas en código validado, declarado en el plan). **Studio**: popover Cielo (tecla 8) con pestañas «Clásico» (comportamiento histórico exacto) / «Realista» (slider hora 0–24 + etiqueta HH:MM, avance 0/10′/20′/60′, checkbox sombras, inclinación 0–90°; escritura por `doc.setSky` con `type:EditableSky` ampliado; `Serializer` ya viajaba `sky` — se tipa `ProjectJson.world.sky`). Demo: import map con `three/addons/`. Tests: `test/engine/daylight.test.js` (7) + 6 en `validate.test.js` (contrato por estilo, retrocompatibilidad, campos cruzados) + `studio/tests/sky-style.test.ts` (4: escritura, sin set/frame en realista, round-trip Serializer, clásico sin style). Pendiente (anotado): luces de punto/fog por interior, fases lunares, nubes | ✅ Realizada |
