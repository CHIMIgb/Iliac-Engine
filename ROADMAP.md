...
| F5 Asset Pipeline (Asset Manager, Sprite Pipeline) | ⏳ Pendiente (Fase A del Sprite Tool ✅: slicer Auto + Manual — Sprite Tool modal con Paso 1 Cargar y Paso 2 Cortar; Auto = detectSprites con sliders minPixels/gapTolerance + trim, Manual = gridSlice cols×rows/spacing/trailingEmpty con celda derivada; preview numerado en vivo, recorte no destructivo a frames en memoria `textureKeyFor(assetId,i)`, thumbs; **Fase B ✅: animador Paso 3** — plantilla idle/walk/attack/death, drag & drop de frames, preview ▶/⏸ fps/loop/step, nueva anim, `buildSpriteAnims` validado con `validateProject` real, guardado real (POST `/assets/sprites/upload` por frame → `doc.setWorldTextures` + `setSpriteAnims`) y puente "Asignar a sprite del mundo ▾"→`sprite.anim`; motor extendido en 6b: `SpriteSystem` animado (`SpriteAnimator.update(dt)`), `Engine3D` orquesta, `validate.js` valida spriteAnims (≥2 frames existentes, fps>0, loop bool); cifras 16 suites/192 tests Studio + 187 motor; **queda Fase C: biblioteca de PNGs sueltos**) |
