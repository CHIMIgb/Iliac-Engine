...
| F4+ **Cielo Daggerfall (tecla 8)**: horizonte lejano `world.sky {set:0–30, frame?:0–31}` — técnica clásica de telón 2D (billboard único FIJO — no cambia al girar el yaw, se retiró el scroll de 32 fotogramas; altura angular natural del fotograma 512×220 sin deformar, z-buffer; sin skybox 3D); DOS subherramientas independientes en el popover: «Horizonte» (carpeta SKY00–SKY30) y «Hora del día» (franja 0–31 dentro del set, cambio de frame sin recargar texturas); `engine/core/sky.js` con `SKY_SETS`, `SKY_FRAMES`, `skyFrameLabel()`; `npm run setup:sky` importa los assets (no versionados); demo con SKY15 | ✅ Realizada |
