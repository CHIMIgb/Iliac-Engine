...
2. **Recarga en vivo:** `EditorState.onChange` → throttle (`reloadMs()`: 120 ms, 250 ms si > 40.000 sectores) → `validateProjectJson` (contrato) → `viewport.reload(raw)` → `Engine3D.setWorld(project)` **sin recrear el renderer** (camino barato si solo cambió el mundo) o motor nuevo si cambió de valor el bloque `render` **o llegaron texturas nuevas** (la comparación usa `renderSignature()` + `worldTextureSignature()`, firmas estables insensibles al orden de claves que impone JSONB; las texturas nuevas exigen el camino caro porque `setWorld` no las recodifica — si no, `SpriteSystem` salta el sprite animado y queda invisible). En el camino caro el motor viejo se **libera antes** de crear el nuevo: dos `WebGLRenderer` vivos sobre el mismo canvas comparten contexto y dejan el viewport (y sus herramientas) congelado.
...
| — | Sprites | Modal Sprite Tool (slicer + animator + biblioteca + mis sprites) |
...
**Reload en vivo:** `doc.onChange` → throttle → validar → `viewport.reload(raw)`. Si solo cambió el `world` (sin texturas nuevas) → `engine.setWorld` (no recrea renderer); si cambió de valor el bloque `render`/cielo **o llegaron texturas nuevas** (los frames al guardar una animación; `setWorld` no las recodifica) → **un solo motor vivo**: `dispose()` del viejo → `new Engine3D` → `load()` (con `renderSignature()` + `worldTextureSignature()` para decidir, y `viewport.onError` → toast si el nuevo no carga). El documento se vuelca con `EditorState.applyFrom(state)`, que **conserva los suscriptores** (`onChange`) del documento vivo.
...
- **Biblioteca (Fase D, index 3) — solo lectura con borrado (Fase G):** animaciones guardadas del **proyecto** (`doc.getSpriteLibrarySnapshot()` → texturas + `spriteAnims`; botón **Refrescar** `refresh-cw` para repintar sin cambiar de tab). Acciones por card: «Asignar a sprite…» (D4, reutiliza el puente `onAssignSprite` → `assignEntityAnim` — **exige la anim guardada**: `assignEntityAnim`/`assignSpriteAnim` devuelven false si la anim no existe en `world.spriteAnims`, así el proyecto nunca queda inválido ni `scheduleReload` aborta, bug 2026-09-18), «Cargar al animador» (D5, reconstruye frames desde las texturas y salta al Paso 3) y **«Eliminar»** (Fase G, icono papelera → `window.confirm` → `onDeleteAnim` inyectado por main.ts → `EditorState.removeSpriteAnim(name)`: borra la anim de `world.spriteAnims`, **los sprites del mundo con `anim === name`** y las texturas de sus frames que queden huérfanas — ni otra anim ni otro sprite las usan; luego `saveCurrent(true)` persiste y la Biblioteca se repinta). Vive fuera del wizardBody (hija del modal).
- **Mis Sprites (Fase F, index 4) — solo lectura:** sprites físicos de la cuenta vía `GET /api/assets?tipo=sprite` (callback `onListMySprites` inyectado por main.ts → `listSpriteAssets`). Cards con thumb (URL pública del blob `assetUrl(id)`), «Cargar al animador» (descarga el blob con `loadPixelFromDataUrl`, lo añade como frame suelto con key derivada del nombre, saltos de colisión `_2`/`_3`…, y salta al Paso 3) y «Eliminar» (confirm() → `onDeleteSprite` → `deleteSpriteAsset` → refresca la lista; ⚠️ el proyecto que use esa textura la pierde). Sin sesión → mensaje de error en la tab.
...
├── storage/uploads/     # Blobs <userId>/<tipo>/<assetId>.<ext> (gitignored, solo .gitkeep)
...
- **Assets:** MIME detectado por magic bytes (`file-type`), nunca por extensión/header; tope 20 MB (413 `ASSET_TOO_LARGE`); **dedupe por hash POR USUARIO** (ASSET_UPLOAD_PLAN.md 2026-09-18): re-subir el mismo archivo en la misma cuenta reutiliza (200), en otra cuenta crea fila y blob propios; blobs en `storage/uploads/<userId>/<tipo>/<assetId>.<ext>` (migración one-shot: `npm run migrate:uploads`).
