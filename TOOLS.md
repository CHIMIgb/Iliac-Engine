...
- **Asignar anim a una ENTIDAD (Fase E, corregida 2026-09-18)**: las animaciones viven sobre entidades colocadas, nunca sobre sprites sueltos. Flujo: (1) coloca la entidad con la herramienta **Entidades** (el tamaño y la colisión vienen del catálogo); (2) en la Sprite Tool → **Biblioteca**, el select lista las **entidades del mundo por nombre** (ej. «Lobo (sp_x)»); (3) botón **«Asignar a entidad»** (`assignEntityAnim`): la entidad adopta la anim por completo — escribe `sprite.anim`, cambia su textura al **primer frame** de la anim y ajusta `sprite.scale` a la **altura de su caja de colisión** (`collisionBox.h`) para que el billboard se adapte equitativamente a su tamaño en el viewport y en el playtest. En el Paso 3, «Asignar anim activa» usa el mismo puente pero tolera la anim **local** del animador aún sin guardar (escribe solo `anim`; la adopción tex/escala exige la anim guardada). Si no hay entidades en el mundo, la Biblioteca lo indica y remite a la herramienta Entidades. El antiguo flujo de «Colocar en el mundo ▾» (sprites nuevos desde la Sprite Tool, E1/E2) se **revirtió** por directiva del usuario.
