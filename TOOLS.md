...
- **Colocar sprites con anim (Fase E, E1)**: el puente `spriteTool.onPlaceSprite(tex, anim)` crea un sprite **NUEVO** con tex+anim en el punto de colocación — el **centro del viewport** proyectado al suelo (lo que estás mirando) — apoyado en el terreno y con snap al grid, y lo **selecciona** para mover/escalar (`EditorViewport.centerWorld()`, `addSprite(..., { anim })`). El botón visible «Colocar en el mundo ▾» llega en E2; hoy el flujo lo puede probar el código conectando el callback.
