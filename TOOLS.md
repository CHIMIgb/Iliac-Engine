...
- **Colocar sprites con anim (Fase E, E1+E2)**: botón **«Colocar en el mundo ▾»** en el Paso 3 (junto a «Asignar anim activa») y en cada card de la Biblioteca. Despliega un menú con las anims **guardadas del proyecto** y crea un sprite **NUEVO** con tex+anim en el punto de colocación — el **centro del viewport** proyectado al suelo (lo que estás mirando) — apoyado en el terreno y con snap al grid, y lo **selecciona** para mover/escalar (`EditorViewport.centerWorld()`, `addSprite(..., { anim })`, puente `onPlaceSprite`). Solo coloca anims guardadas: las locales del animador no existen en el proyecto hasta pulsar «Guardar en el proyecto».
