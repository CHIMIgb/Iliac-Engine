...
- **Brújula HUD** (`engine/three/CompassOverlay.js`): rosa N/E/S/O fija en la esquina inferior derecha (canvas 2D, sin Three); **la dibuja el motor**, la activa `Engine3D.setCompass(true)` (el Studio la enciende al entrar en playtest; la demo también). Se rota con el yaw del jugador (`compassRotation`) y su norte coincide con el polo de la aurora.
